...
{
	if (m.title && m.year && m.description && m.genres && m.votes && m.rating && m.moviedb_id && m.imdb_id && m.poster_file && m.backdrop_file)
	{
		_logger.info('Using exinsting metadata for Movie (#%d): %s (%s)', m.id, info.title, info.year || 'unknown year');
		m.state = 'ready';
	...
	_async.series([
		...
		function (cb)
		{
			...
			_db.Media.find({where : { moviedb_id : movieinfo.id }}).complete(function(err, res)
			{
				...
				if (res && res.id != m.id)
					return _discrep.dupe(m, (res.abspath || ('#' + res.id)), false, cbfind);
				else
					return cb();
			});
		},
