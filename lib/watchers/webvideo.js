...
{
	var req = _request(s.url)
	var parser = new _fp();
	var items = [];

	_logger.info('Downloading webvideo RSS from %s', name);

	req.on('error', function (err)
	{
		_logger.err('Error trying to retrieve %s: %s', s.url, err);
	});
	...
	{
		_logger.err('Error parsing %s: %s', s.url, err);
	});
