...
function init()
{
	...
	files.forEach(function(f)
	{
		...
		if (!_conf.watchers.webvideo.sources || !_conf.watchers.webvideo.sources[name] || !_conf.watchers.webvideo.sources[name].enabled)
			return;
...
function saveAttr(name, guid, attr, cb)
{
	_db.Media.findOrInitialize({ hash : guid }).complete(function(err, m, finit)
	{
		...
		if (attr.poster)
		{
			...
			_helper.downloadImage(attr.poster, filename, 'Poster for ' + attr.title, function(err)
			{
				if (err)
					return cb(err);
...
function refresh(cb)
{
	_async.each(Object.keys(_sources), function(name, cb)
	{
		var s = _sources[name];
		
		var cfg = _conf.watchers.webvideo.sources[name].config || {};
