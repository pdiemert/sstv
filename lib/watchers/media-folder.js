	...
	function finish(ep, cb)
	{
		_db.Series.find({where: { tvdb_id: series_tvdb_id }}).complete(function (err, s)
		{
			...
			if (ep && ep.poster_file)
			{
				...
				var filename = 'episode_' + imageHash(m.title, _moment([m.airyear, m.airmonth - 1, m.airday]).toString()) + '_poster.' + ep.poster_file.substr(ep.poster_file.length - 3);
				_helper.downloadImage('http://thetvdb.com/banners/' + ep.poster_file, filename, function(err)
				{
					if (err)
						return cb(err);
	...
	_async.waterfall([
		...
		}, function (ep, cb)
		{
			...
			_async.each(images, function (e, cb)
				{
					_helper.downloadImage(e.url, e.filename, cb);
				}, function (err)
				{
					if (err)
						return cb(err);
...
function findMovieMetadataFromName(m, info, cfg, cbfind)
{
	...
	_async.series([
		...
		function (cb)
		{
			...
			_helper.downloadImage(url, poster, cb);
		...
		}, function (cb)
		{
			...
			if (movieinfo.backdrop_path)
			{
				...
				_helper.downloadImage(url, backdrop, cb);
			}
			else
				return cb();
