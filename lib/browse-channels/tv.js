...
function getItems(path, cb)
{
	...
	else
	{
		if (path.length == 1)
		{
			...
			{
				var list = res.map(function(e) { return { type : 'Section', title : e.title, id : e.id, thumb : e.poster_file ? _path.join(_conf.image_dir, e.poster_file) : null }; });
		...
		else if (path.length == 2)
		{
			...
			{
				var list = res.map(function(e) { return { type : 'Section', title : 'Season ' + e.season_number , id : e.season_number, thumb : _conf.image_dir ? _path.join(_conf.image_dir, e.poster_file) : null }; });
			...
			_db.Media.findAll({where : { series_tvdb_id : path[1], season_number : path[2] }, order : 'episode_number'}).complete(function(err, res)
			{
				...
				var list = res.map(function(e)
				{
					var item = {
						title : e.title,
						id : e.id,
						thumb : _path.join(_conf.image_dir, e.poster_file),
						media_id : e.id
					};
