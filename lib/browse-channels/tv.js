...
function getItems(path, cb)
{
	...
	else
	{
		if (path.length == 1)
		{
			...
			{
				var list = res.map(function(e) { return { title : e.title, id : e.id, thumb : e.poster_file ? _path.join(_conf.image_dir, e.poster_file) : null }; });
		...
		else if (path.length == 2)
		{
			...
			{
				var list = res.map(function(e) { return { title : 'Season ' + e.season_number , id : e.season_number, thumb : _conf.image_dir ? _path.join(_conf.image_dir, e.poster_file) : null }; });
